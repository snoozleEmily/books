-- PLSQL
-- Create table for Chakra Natures
...
(3, 'air', 'fūton', '風'), -- ID: 3
(4, 'earth', 'doton', '土'), -- ID: 4
(5, 'lighting', 'raiton', '雷'), -- ID: 5
(6, 'yin', 'in', '陰'), -- ID: 6
(7, 'yang', 'yō', '陽'); -- ID: 7


-- Create chakra type relationships table
...
-- Insert data into the chakra relationships table
INSERT INTO chakra_relations (attacker_element_id, defender_element_id, relation_type) VALUES
(1, 2, 'weak'),    -- Fire < Water
(1, 3, 'strong'),  -- Fire > Air
...
SELECT COALESCE(relation_type, 'equal') AS relation_type FROM chakra_relations
FROM chakra_relations
WHERE attacker_chakra_id = :player_element_id
AND defender_element_id = :enemy_element_id
...
-- Insert data into the Jutsus table
-- Fire Jutsus
